 ...
 * <p>跨 announcement_reads / user_tenant_mapping / users / announcement_depts 數個表，
 * 用 JPA derived query 難以高效表達，故集中使用 native query。
 * <p>租戶過濾以參數帶入而非依賴 @Filter（native query 不受 Hibernate filter 影響）。
 */
public interface AnnouncementStatsRepository extends Repository<com.taipei.iot.announcement.entity.AnnouncementRead, Long> {
    ...
    @Query(value = """
            ...
            """, nativeQuery = true)
    long countAudienceDept(@Param("tenantId") String tenantId,
                           @Param("deptIds") Collection<Long> deptIds);

    /**
     * scope=ALL 公告之已讀人數（限受眾範圍內 = 仍在租戶且啟用）。
     * <p>離職或被停用的使用者不計入，避免比例失真。
     */
    @Query(value = """
            ...
            """, nativeQuery = true)
    long countReadAll(@Param("announcementId") Long announcementId,
    ...
    @Query(value = """
            ...
            WHERE r.announcement_id = :announcementId
              ...
              AND m.dept_id IN (:deptIds)
            """, nativeQuery = true)
    long countReadDept(@Param("announcementId") Long announcementId,
                       @Param("tenantId") String tenantId,
                       @Param("deptIds") Collection<Long> deptIds);

    /**
     * scope=ALL 未讀使用者清單（分頁）。
     * <p>欄位順序：user_id, display_name, email, dept_id, dept_name。
     * 採 LEFT JOIN dept_info 容許未綁定部門之使用者仍可列出。
     */
    @Query(value = """
            ...
            WHERE m.tenant_id = :tenantId AND m.enabled = true AND u.deleted = false
              ...
              AND (CAST(:keyword AS text) IS NULL
                   ...
                   OR LOWER(u.email) LIKE LOWER(CONCAT('%', CAST(:keyword AS text), '%')))
            """, nativeQuery = true)
    Page<Object[]> findUnreadUsersAll(@Param("announcementId") Long announcementId,
                                      @Param("tenantId") String tenantId,
    ...
    @Query(value = """
            ...
            WHERE m.tenant_id = :tenantId AND m.enabled = true AND u.deleted = false
              ...
              AND (CAST(:keyword AS text) IS NULL
                   ...
                   OR LOWER(u.email) LIKE LOWER(CONCAT('%', CAST(:keyword AS text), '%')))
            """, nativeQuery = true)
    Page<Object[]> findUnreadUsersDept(@Param("announcementId") Long announcementId,
                                       @Param("tenantId") String tenantId,
                                       @Param("deptIds") Collection<Long> deptIds,
                                       @Param("keyword") String keyword,
                                       Pageable pageable);
