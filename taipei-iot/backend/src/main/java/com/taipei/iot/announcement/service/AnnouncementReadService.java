...
public class AnnouncementReadService {
     ...
     * <p>權限：呼叫者需具 ANNOUNCEMENT_MANAGE（由 controller 控制）；
     * DEPT_ADMIN 僅能查看自建公告之統計，避免越權看到其他部門名單。
     */
    @Transactional(readOnly = true)
    public AnnouncementReadStatsResponse getReadStats(Long announcementId) {
        ...
        List<Long> deptIds = isAll
                ? Collections.emptyList()
                : announcementDeptRepository.findByAnnouncementId(announcementId).stream()
                    .map(AnnouncementDept::getDeptId).toList();
     ...
     */
    @Transactional(readOnly = true)
    public PageResponse<AnnouncementUnreadUserResponse> getUnreadUsers(
            Long announcementId, String keyword, int page, int size) {
        Announcement entity = loadAndCheckManage(announcementId);
        String tenantId = TenantContext.getCurrentTenantId();
        ...
        if (AnnouncementScope.ALL.getValue().equals(entity.getScope())) {
            ...
            List<Long> deptIds = announcementDeptRepository.findByAnnouncementId(announcementId).stream()
                    .map(AnnouncementDept::getDeptId).toList();
            if (deptIds.isEmpty()) {
                return PageResponse.<AnnouncementUnreadUserResponse>builder()
                        .content(Collections.emptyList())
                        .totalElements(0).totalPages(0).page(page).size(size)
                        .build();
            }
            rows = announcementStatsRepository.findUnreadUsersDept(announcementId, tenantId, deptIds, safeKeyword, pageable);
        }

        List<AnnouncementUnreadUserResponse> content = rows.getContent().stream()
    ...
    /**
     * 載入公告並檢查當前使用者是否有權查看統計：
     * ADMIN（DataScope=ALL）可看全部；DEPT_ADMIN 僅能看自建公告。
     */
