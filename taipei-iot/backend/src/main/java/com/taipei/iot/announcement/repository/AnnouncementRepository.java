...
public interface AnnouncementRepository extends JpaRepository<Announcement, Long>, TenantScopedRepository {
     ...
     * <p>可另外限定 category；:category 為 null 表示不過濾。
     */
    @Query("""
    ...
    /**
     * 管理頁面：ADMIN 看全部
     * <p>statusFilter 支援：ALL / DRAFT / SCHEDULED / PUBLISHED / EXPIRED。
     * 其中 SCHEDULED 為計算狀態：status=PUBLISHED 但 publishAt > now。
     */
    @Query("""
        SELECT a FROM Announcement a
    ...
    @Query("""
        ...
        """)
    java.util.List<Announcement> findPinnedForDeptAdmin(
            @Param("userId") String userId,
            @Param("userDeptId") Long userDeptId);
