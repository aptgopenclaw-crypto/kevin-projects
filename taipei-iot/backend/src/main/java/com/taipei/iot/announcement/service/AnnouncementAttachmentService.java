...
/**
 * 公告附件服務。
 * <p>沿用 {@link FileStorageService} / {@link FileValidationService} 做檔案落地與校驗，
 * 中繼資料寫入 {@code announcement_attachments}。
 */
@Slf4j
@Service
public class AnnouncementAttachmentService {
    ...
    /**
     * 公告附件專用副檔名白名單（全小寫）。
     * <p>預設僅允許 PDF。可透過 {@code announcement.attachments.allowed-extensions} 或
     * 環境變數 {@code ANNOUNCEMENT_ATTACHMENT_EXTS} 覆寫。
     * <p>此名單為 {@link FileValidationService} 全域白名單的縮小子集，
     * 並在全域驗證通過後额外套用，拒絕 SVG/HTML/Office macro 檔 等高風險類型。
     */
    ...
    public AnnouncementAttachmentService(
            ...
            AnnouncementDeptRepository announcementDeptRepository,
    ...
    public Map<Long, List<AnnouncementAttachmentResponse>> listByAnnouncementIds(List<Long> announcementIds) {
        if (announcementIds == null || announcementIds.isEmpty()) return Collections.emptyMap();
        return attachmentRepository.findByAnnouncementIdInOrderByIdAsc(announcementIds).stream()
                .collect(Collectors.groupingBy(
                        AnnouncementAttachment::getAnnouncementId,
    ...
    public AnnouncementAttachmentResponse upload(Long announcementId, MultipartFile file) {
        ...
        log.info("Announcement attachment uploaded: announcementId={}, attachmentId={}, file={}",
                announcement.getId(), entity.getId(), entity.getFileName());
    ...
    public void delete(Long announcementId, Long attachmentId) {
        ...
        log.info("Announcement attachment deleted: announcementId={}, attachmentId={}",
                announcement.getId(), attachmentId);
    }
    ...
    private boolean hasManagePermission() {
        ...
        return auth.getAuthorities().stream()
                .anyMatch(a -> "ANNOUNCEMENT_MANAGE".equals(a.getAuthority()));
    }
