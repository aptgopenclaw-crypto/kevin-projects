...
class ErrorCodeTest {
    ...
    void allErrorCodes_areUnique() {
        ...
        assertEquals(ErrorCode.values().length, distinctCount,
                "ErrorCode.code 出現重複值，請檢查 enum 定義。" +
                "重複的 code 會造成 client 端無法區分不同錯誤類型。");
    }
    ...
    void allErrorCodes_httpStatusIsValidHttpRange() {
        Arrays.stream(ErrorCode.values()).forEach(code ->
                assertTrue(code.getHttpStatus() >= 200 && code.getHttpStatus() < 600,
    ...
    void allErrorCodes_haveNonBlankMessage() {
        Arrays.stream(ErrorCode.values()).forEach(code ->
                assertNotNull(code.getMessage(),
                        code.name() + " 的 message 不應為 null")
        );
        Arrays.stream(ErrorCode.values()).forEach(code ->
                assertFalse(code.getMessage().isBlank(),
                        code.name() + " 的 message 不應為空字串")
        );
    }
}
