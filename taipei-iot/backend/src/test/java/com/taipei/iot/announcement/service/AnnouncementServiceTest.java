...
class AnnouncementServiceTest {
    ...
    private Announcement buildAnnouncement(Long id, String status, String scope,
                                            String createdBy, LocalDateTime publishAt,
    ...
    class CreateTests {
        ...
        void create_asAdmin_scopeAll_succeeds() {
            ...
            AnnouncementRequest req = AnnouncementRequest.builder()
                    .title("New").content("Body").status("PUBLISHED")
        ...
        void create_asDeptAdmin_forceScopeToDept() {
            ...
            when(announcementDeptRepository.findByAnnouncementId(2L)).thenReturn(
                    List.of(AnnouncementDept.builder().announcementId(2L).deptId(5L).build()));
            when(announcementReadRepository.findByAnnouncementIdInAndUserId(any(), any()))
                    .thenReturn(List.of());
            ...
            AnnouncementRequest req = AnnouncementRequest.builder()
                    .title("Dept News").content("Body").status("PUBLISHED")
            ...
            verify(announcementDeptRepository).saveAll(argThat(list ->
                    ((List<?>) list).size() == 1));
        }
        ...
        void create_deptScope_withoutDeptIds_throwsValidationError() {
            ...
            AnnouncementRequest req = AnnouncementRequest.builder()
                    .title("Dept News").content("Body").status("PUBLISHED")
                    .scope("DEPT").targetDeptIds(List.of()) // 空 list
                    .pinned(false).build();
    ...
    class UpdateTests {
        ...
        void update_asDeptAdmin_notOwner_throwsPermissionDenied() {
            ...
            Announcement existing = buildAnnouncement(1L, "DRAFT", "DEPT",
                    "other-user", LocalDateTime.now(), null);
            when(announcementRepository.findById(1L)).thenReturn(Optional.of(existing));
    ...
    class DeleteTests {
        ...
        void delete_asAdmin_succeeds() {
            setSecurityContext("admin-1", 1L, "ALL");
            Announcement existing = buildAnnouncement(1L, "PUBLISHED", "ALL",
                    "someone", LocalDateTime.now(), null);
            when(announcementRepository.findById(1L)).thenReturn(Optional.of(existing));
        ...
        void delete_asDeptAdmin_notOwner_throwsPermissionDenied() {
            setSecurityContext("dept-admin-1", 5L, "THIS_LEVEL");
            Announcement existing = buildAnnouncement(1L, "PUBLISHED", "DEPT",
                    "other-user", LocalDateTime.now(), null);
            when(announcementRepository.findById(1L)).thenReturn(Optional.of(existing));
        ...
        void delete_asDeptAdmin_owner_succeeds() {
            setSecurityContext("dept-admin-1", 5L, "THIS_LEVEL");
            Announcement existing = buildAnnouncement(1L, "PUBLISHED", "DEPT",
                    "dept-admin-1", LocalDateTime.now(), null);
            when(announcementRepository.findById(1L)).thenReturn(Optional.of(existing));
    ...
    class GetByIdTests {
        ...
        void getById_withManagePermission_canSeeDraft() {
            setSecurityContext("admin-1", 1L, "ALL");
            Announcement entity = buildAnnouncement(1L, "DRAFT", "ALL",
                    "admin-1", LocalDateTime.now(), null);
        ...
        void getById_withoutPermission_publishedVisible_succeeds() {
            ...
            Announcement entity = buildAnnouncement(1L, "PUBLISHED", "ALL",
                    "admin-1", past, future);
            when(announcementRepository.findById(1L)).thenReturn(Optional.of(entity));
            when(announcementDeptRepository.findByAnnouncementId(1L)).thenReturn(List.of());
            when(announcementReadRepository.findByAnnouncementIdInAndUserId(any(), any()))
        ...
        void getById_withoutPermission_draftNotVisible_throwsNotFound() {
            ...
            BusinessException ex = assertThrows(BusinessException.class,
                    () -> announcementService.getById(1L, false));
        ...
        void getById_withoutPermission_deptScope_notInDept_throwsNotFound() {
            ...
            LocalDateTime past = LocalDateTime.now().minusHours(1);
            LocalDateTime future = LocalDateTime.now().plusDays(10);
            Announcement entity = buildAnnouncement(1L, "PUBLISHED", "DEPT",
                    "admin-1", past, future);
            when(announcementRepository.findById(1L)).thenReturn(Optional.of(entity));
            when(announcementDeptRepository.existsByAnnouncementIdAndDeptId(1L, 3L))
                    .thenReturn(false);
        ...
        void getById_notFound_throwsException() {
            ...
            assertThrows(BusinessException.class,
                    () -> announcementService.getById(99L, false));
        }
    }
    ...
    class ListVisibleTests {
        ...
        void listVisible_returnsPageResponse() {
            ...
            when(announcementDeptRepository.findByAnnouncementIdIn(List.of(1L)))
                    .thenReturn(List.of());
            when(announcementReadRepository.findByAnnouncementIdInAndUserId(any(), eq("user-1")))
                    .thenReturn(List.of());
