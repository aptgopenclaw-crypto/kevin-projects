...
public class SystemSettingController {
    ...
    @GetMapping
    @PreAuthorize("hasAnyRole('ADMIN', 'SUPER_ADMIN') or hasAuthority('SYSTEM_SETTINGS_VIEW')")
    public BaseResponse<List<SystemSettingDto>> listSettings() {
        return BaseResponse.success(settingService.findAllSettings());
    }

    @PutMapping("/{key}")
    @PreAuthorize("hasAnyRole('ADMIN', 'SUPER_ADMIN') or hasAuthority('SYSTEM_SETTINGS_MANAGE')")
