...
public class WorkflowServiceImpl implements WorkflowService {
    ...
    private static final Set<String> TERMINAL_STEPS = Set.of(
        "CONFIRMED", "REJECTED", "MERGED", "CLOSED", "APPLIED", "TRANSFERRED"
    );

    private static final Map<String, String> INITIAL_STEPS = Map.of(
        "FAULT_REVIEW",       "OPEN",
        "REPAIR_DISPATCH",    "PENDING",
        "REPAIR_CLOSE",       "COMPLETION_REPORTED",
        "REPLACEMENT_REVIEW", "DRAFT",
        "ASSET_CHANGE",       "DRAFT"
    );
    ...
    @Override
    @Transactional
    public void transition(Long instanceId, String targetStep, String action,
                           String actorId, String actorName, String comment,
        ...
        String finalComment = isDelegated
                ? String.format("[代理簽核] %s。%s", actorName, comment != null ? comment : "")
                : comment;
    ...
    public Page<WorkflowInstanceResponse> getMyPendingTasks(String userId, Pageable pageable) {
        ...
        // 查詢誰把我設為代理人（且日期有效）
        List<DelegateSetting> activeDelegations = delegateSettingRepository
                .findActiveDelegationsForDelegate(userId, LocalDate.now());
        for (DelegateSetting ds : activeDelegations) {
            assigneeIds.add(ds.getDelegatorId());
        }

        Page<WorkflowInstance> instances = instanceRepository
                .findPendingByAssignees(assigneeIds, WorkflowStatus.ACTIVE, pageable);
    ...
    public List<WorkflowStepLogResponse> getStepLogs(Long instanceId) {
        return stepLogRepository.findByInstanceIdOrderByActedAtAsc(instanceId)
                .stream().map(this::toStepLogResponse).toList();
    }
