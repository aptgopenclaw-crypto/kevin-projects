...
class NotificationControllerTest {
    ...
    void list_shouldReturn200ForAuthenticatedUser() throws Exception {
        ...
        mockMvc.perform(get("/v1/auth/notifications")
    ...
    void listTodos_shouldReturn200() throws Exception {
        ...
        mockMvc.perform(get("/v1/auth/notifications/todos")
                        .header("Authorization", "Bearer " + validToken())
                        .param("page", "0")
                        .param("size", "20"))
                .andExpect(status().isOk())
    ...
    void unreadCount_shouldReturn200() throws Exception {
        ...
        when(notificationService.unreadCount("u1"))
                .thenReturn(UnreadCountResponse.builder().count(5).build());

        mockMvc.perform(get("/v1/auth/notifications/unread-count")
                        .header("Authorization", "Bearer " + validToken()))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.errorCode").value("00000"))
    ...
    void markRead_shouldReturn200() throws Exception {
        ...
        mockMvc.perform(patch("/v1/auth/notifications/1/read")
                        .header("Authorization", "Bearer " + validToken()))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.errorCode").value("00000"));
    ...
    void markAllRead_shouldReturn200() throws Exception {
        ...
        mockMvc.perform(patch("/v1/auth/notifications/read-all")
                        .header("Authorization", "Bearer " + validToken()))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.errorCode").value("00000"));
