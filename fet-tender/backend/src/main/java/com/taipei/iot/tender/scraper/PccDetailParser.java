...
/**
 * 政府採購網詳細頁 HTML 解析器（對應 Python v3/src/detail_parser.py）。
 *
 * 解析邏輯：
 *   1. 依 table summary 屬性定位各區塊（機關資料、採購資料、招標資料、領投開標、其他）
 *   2. label cell：class 含 tbg_1/tbg_4/tbg_5/tbg_6/tbg_7
 *      value cell：class 含 tbg_2/tbg_4R
 *   3. 補充以 HTML id 直接抓取特定欄位（更穩定）
 *   4. 開標時間/地點：尋找對應 td 文字後取 next sibling
 */
public class PccDetailParser {
    ...
    private static void parseTableSection(Document doc, String sectionName, Map<String, String> result) {
        // Jsoup attribute selector: table[summary='機關資料']
