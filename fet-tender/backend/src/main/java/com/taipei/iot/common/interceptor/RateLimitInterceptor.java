 ...
 *
 * <p>原理：使用 Redis INCR + EXPIRE 實作固定窗口計數器。
 * <ul>
...
public class RateLimitInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response,
        ...
        if (currentCount > rateLimit.limit()) {
            SecurityLogger.warn(SecurityEvent.RATE_LIMITED, clientIp,
                    "endpoint=" + rateLimit.key(),
                    "count=" + currentCount + "/" + rateLimit.limit(),
                    "period=" + rateLimit.period() + "s");
            log.warn("速率限制觸發 — IP: {}, key: {}, count: {}/{}, period: {}s",
                    clientIp, rateLimit.key(), currentCount, rateLimit.limit(), rateLimit.period());
            ...
            String body = objectMapper.writeValueAsString(
                    BaseResponse.fail(ErrorCode.RATE_LIMIT_EXCEEDED));
     ...
     *
     * <h3>安全考量（CVE 級別）：</h3>
     * <p>X-Forwarded-For、X-Real-IP 等 HTTP header 是<b>客戶端可任意偽造的</b>。
     * 如果速率限制依賴這些 header 來識別 IP，攻擊者只需每次請求帶不同的
     * {@code X-Forwarded-For} 值，就能無限繞過速率限制：</p>
     * <pre>
     ...
     *
     * <h3>修復策略：</h3>
     * <p>直接使用 {@code request.getRemoteAddr()} — 這是 TCP 連線層的來源 IP，
     * 無法被 HTTP header 偽造。只有在前方有<b>受信任的反向代理</b>（如 Nginx）
     * 且由 Nginx 設定 X-Forwarded-For 時，才應讀取 proxy header。</p>
     *
     * <p>目前架構：Client → Tomcat（無反向代理），因此 remoteAddr 就是真實客戶端 IP。
     * 未來若部署 Nginx，應搭配 Spring 的 {@code ForwardedHeaderFilter}
     * 或在 {@code application.yml} 設定 {@code server.forward-headers-strategy=NATIVE}，
     * 由框架層安全處理，而非手動讀取 header。</p>
     */
